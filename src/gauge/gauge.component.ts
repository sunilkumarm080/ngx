...
export class GaugeComponent extends BaseChartComponent implements AfterViewInit {
  ...
  update(): void {
    ...
    const xOffset = (this.margin[2] + this.dims.width / 2) - 65;
  ...
  getArcs(): any[] {
    const arcs = [];
    const availableRadius = this.outerRadius * .7;
    const radiusPerArc = Math.min(availableRadius / this.results.length, 10);
    const arcWidth = radiusPerArc * 1.5;
    this.textRadius = this.outerRadius - this.results.length * radiusPerArc;
    this.cornerRadius = Math.floor(arcWidth / 3);

    let i = 0;
    for (const d of this.results) {
      const outerRadius = this.outerRadius - (i * radiusPerArc * 2.5);
